...
class PostForm(forms.ModelForm):
    name = forms.CharField(label=ugettext_lazy('Subject'), widget=forms.TextInput(attrs={'class': 'textinput textInput form-control'}))
    poll_type = forms.TypedChoiceField(label=ugettext_lazy('Poll type'), choices=Topic.POLL_TYPE_CHOICES, coerce=int, widget=forms.Select(attrs={'class': 'form-control'}))
    poll_question = forms.CharField(
        label=ugettext_lazy('Poll question'),
        required=False,
        widget=forms.Textarea(attrs={'class': 'form-control markItUpEditor'}))
...
class AdminPostForm(PostForm):
    ...
    """
    login = forms.CharField(label=ugettext_lazy('User'), widget=forms.TextInput(attrs={'class': 'textinput textInput form-control'}))
...
try:
    class EditProfileForm(forms.ModelForm):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.fields['signature'].widget = forms.Textarea(attrs={'rows': 2, 'cols:': 60}, widget=forms.Textarea(attrs={'class': 'form-control markItUpEditor'}))
